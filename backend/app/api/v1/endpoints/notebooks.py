...
async def update_notebook(
    notebook_id: str, update_data: NotebookUpdate, db: Session = Depends(get_db)
):
    """Update a notebook"""
    try:
...
async def delete_notebook(notebook_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        return APIResponse(
            success=True, message=f"Notebook {notebook_id} deleted successfully"
        )
    except HTTPException:
        raise
    except Exception as e:
...
async def add_cell(notebook_id: str, cell: NotebookCell, db: Session = Depends(get_db)):
...
async def delete_cell(notebook_id: str, cell_id: str, db: Session = Depends(get_db)):
...
@router.post(
    "/{notebook_id}/cells/{cell_id}/execute", response_model=CellExecuteResponse
)
async def execute_cell(
    ...
    try:
        user_id = str(current_user.id) if current_user else None
        result = await notebook_service.execute_cell(
            db, notebook_id, cell_id, request.code, user_id
        )
        return result
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
...
async def import_notebook(
    ...
    try:
        ...
        notebook = await notebook_service.import_notebook(
            db, content.decode("utf-8"), file.filename, user_id
        )
        return notebook
    except Exception as e:
...
async def export_notebook(notebook_id: str, db: Session = Depends(get_db)):
