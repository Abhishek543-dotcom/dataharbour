...
class AirflowService:
    ...
    async def get_dag_runs(
        self, dag_id: str, limit: int = 25, offset: int = 0, state: Optional[str] = None
    ) -> Dict[str, Any]:
    ...
    async def trigger_dag(
        ...
        try:
            ...
            response = self._make_request(
                "POST", f"/dags/{dag_id}/dagRuns", json=payload
            )
    ...
    async def pause_dag(self, dag_id: str) -> Dict[str, Any]:
        ...
        try:
            response = self._make_request(
                "PATCH", f"/dags/{dag_id}", json={"is_paused": True}
            )
            return response
        except Exception as e:
    ...
    async def unpause_dag(self, dag_id: str) -> Dict[str, Any]:
        ...
        try:
            response = self._make_request(
                "PATCH", f"/dags/{dag_id}", json={"is_paused": False}
            )
            return response
        except Exception as e:
    ...
    async def get_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_runs = {"running": 0, "success": 0, "failed": 0, "queued": 0}
