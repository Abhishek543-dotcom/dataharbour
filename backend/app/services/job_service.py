...
from app.models.schemas import Job, JobCreate, JobDetails, JobFilter, JobStatus
...
class JobService:
    ...
    async def create_job(
        self, db: Session, job_data: JobCreate, user_id: Optional[str] = None
    ) -> JobDetails:
    ...
    async def _execute_job(self, job_id: str, code: str, cluster_id: str):
        ...
        try:
            ...
            logs = [
                ...
                f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} INFO SparkSession: "
                f"Created SparkSession for {job_db.name}",
            ]
            ...
            result = await asyncio.to_thread(
                spark_service.execute_code, code, cluster_id
            )
            ...
            if (
                ...
                or result["status"] == "completed_with_warnings"
            ):
                update_dict["status"] = JobStatus.COMPLETED.value
                logs.append(
                    f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} INFO DAGScheduler: "
                ...
                logs.append(
                    f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} ERROR SparkContext: Job execution failed"
                )
    ...
    async def restart_job(
        self, db: Session, job_id: str, user_id: Optional[str] = None
    ) -> JobDetails:
        """Restart a job"""
        try:
